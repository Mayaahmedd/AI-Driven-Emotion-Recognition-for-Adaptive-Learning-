...
def make_env(
    ...
    env = StudentEnv(
        ...
        include_persistent_obs=include_persistent_obs,
        obs_space_variant=obs_space_variant,
    )
    env = DQNSafeEnv(env)
    return Monitor(
        env,
...
class DQNAgent(BaseAgent):
    ...
    def verify_obs_compat(self, env: gym.Env) -> None:
        """Raise ValueError if saved model input dim mismatches environment observation space."""
        if self.model is None:
        ...
        if expected != saved:
            raise ValueError(
                ...
                f"Set include_persistent_obs={'True' if saved == 7 else 'False'} "
                f"or retrain with the target observation space."
            )
