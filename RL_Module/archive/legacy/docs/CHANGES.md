...
- **Fix:** Full step/episode logging; DQN ablation; ERT `persistent_flag` passthrough in `evaluate()`.
...
- **Bug:** `--algo` used `choices=ALGO_ORDER` (underscore names only); `argparse` rejected alias names before `AGENT_ALIASES` ran.
...
## Thesis-final cleanup (2026-05-31)

### Bandit+DQN removal
- **Deleted:** `agents/bandit_dqn.py` (`BanditDQNAgent`, LinUCB hybrid).
- **Updated:** `agents/__init__.py`, `evaluation/metrics.py`, `config.py`, `main_experiment.py`, legacy figure generators, `README.md`.
- **Removed constants:** `BANDIT_ALPHA`, `BANDIT_EMOTION_DELTA_THRESHOLD`.
- **Thesis tutors unchanged:** DQN, ERT, Random. See `FINAL_ALGORITHM_STATUS.md` and `BANDIT_REMOVAL_VERIFICATION.md`.

## State-space audit (2026-05-31)

### Partial observability fix
- **Issue:** `persistent_frustration_flag` and `consecutive_flag_steps` drove masking, reward, and dropout but were not in the DQN observation vector.
- **Fix:** Optional 7-dim observation via `include_persistent_obs=True` or `obs_space_variant` presets (`A_original_v6`, `B_v7_with_persistent`, `C_v7_persistent_no_eid`).
- **New:** `build_observation()`, `normalize_persistent_steps()` in `mdp_definition.py`; `StudentState.from_vec()` accepts 6- or 7-dim vectors.
- **Compat:** Default remains 6-dim; `DQNAgent.verify_obs_compat()` guards saved-model loading.
- **Study:** `state_space_ablation_study.py` → `figures/state_space_ablation/`.
- **Docs:** `STATE_SPACE_AUDIT.md`.
