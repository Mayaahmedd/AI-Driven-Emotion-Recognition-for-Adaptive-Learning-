...
This chapter reports the experimental results of the proposed system. It is organised by module, in the same order as the methodology chapter. Part~I, in Sections~\ref{sec:results_fer}--\ref{sec:results_fer_summary}, presents the quantitative results of the Facial Expression Recognition (FER) module on the DAiSEE benchmark \cite{gupta_daisee_2016}. The results of the adaptation module, which consumes the FER output as one of its inputs, are reported in Part~II once the adaptation methodology is fully introduced. The reason for this two-part structure is that the two modules optimise different objectives and use different evaluation protocols: FER is judged with multilabel classification metrics on DAiSEE, while the adaptation module is judged with policy-style metrics inside a tutoring loop.
...
% =====================================================================
% PART II — ADAPTATION RESULTS (placeholder; written in a later iteration)
% =====================================================================

\section{Adaptation Module Results}\label{sec:results_adapt}

The results of the adaptation module are reported here once the adaptation methodology is fully introduced in the methodology chapter. The placeholder is kept so that the chapter outline matches the final intended structure. The FER part above is independent of this section and can be read as a self-contained chapter on the perception module of the system.
