...
from adaptation.step3.config import ARCHETYPES, EXPLANATION_ACTIONS, PRACTICE_ACTIONS, SimConfig
...
class StudentSimulator:
    ...
    @staticmethod
    def to_vector(state: State) -> List[float]:
        return [
            state["engaged"],
            state["confused"],
            state["frustrated"],
            state["bored"],
            state["delta_engaged"],
            state["delta_confused"],
            state["delta_frustrated"],
            state["delta_bored"],
            state["last_correct"],
            state["progress"],
        ]
    ...
    def _transition_explanation(self, action: str) -> Tuple[State, int]:
        ...
        engaged += self.rng.uniform(-0.015, 0.015)
        confused += self.rng.uniform(-0.012, 0.012)
        frustrated += self.rng.uniform(-0.012, 0.012)
        bored += self.rng.uniform(-0.012, 0.012)

        next_state = self._build_next_state(engaged, confused, frustrated, bored, prev["last_correct"])
        return next_state, int(prev["last_correct"])
    ...
    def _transition_practice(self, action: str) -> Tuple[State, int]:
        ...
        difficulty_map = {
            ...
            "question_medium": 0.5,
            "question_hard": 0.8,
            "question_review": 0.35,
        }
        diff = difficulty_map[action]

        stress = prev["frustrated"] + prev["confused"] - prev["engaged"]
        correct_prob = self.config.base_correct_prob + 0.10 * prev["engaged"] - 0.22 * max(0.0, stress)
        correct_prob -= 0.25 * (diff - 0.5)
        correct_prob = self._adjust_correct_prob_for_archetype(correct_prob, action, prev)
        correct_prob = max(0.1, min(0.95, correct_prob))
        correct = 1 if self.rng.random() < correct_prob else 0
        ...
        engaged, confused, frustrated, bored = self._apply_archetype_shift(engaged, confused, frustrated, bored)
        engaged += self.rng.uniform(-0.015, 0.015)
        confused += self.rng.uniform(-0.015, 0.015)
        frustrated += self.rng.uniform(-0.015, 0.015)
        bored += self.rng.uniform(-0.015, 0.015)
        next_state = self._build_next_state(engaged, confused, frustrated, bored, float(correct))
    ...
    def _build_next_state(self, engaged: float, confused: float, frustrated: float, bored: float, last_correct: float) -> State:
        prev = self.state
        normalized = self._normalize_probs(engaged, confused, frustrated, bored)
        return self._normalize(
            {
                "engaged": normalized[0],
                "confused": normalized[1],
                "frustrated": normalized[2],
                "bored": normalized[3],
                "delta_engaged": normalized[0] - prev["engaged"],
                "delta_confused": normalized[1] - prev["confused"],
                "delta_frustrated": normalized[2] - prev["frustrated"],
                "delta_bored": normalized[3] - prev["bored"],
                "last_correct": last_correct,
                "progress": (self.step_idx + 1) / self.config.steps_per_episode,
            }
        )

    def _reward_explanation(self, prev: State, curr: State) -> float:
        return 0.55 * (curr["engaged"] - prev["engaged"]) - 0.25 * max(0.0, curr["confused"] - prev["confused"]) - 0.20 * max(
            0.0, curr["frustrated"] - prev["frustrated"]
        )
    ...
    def _normalize(self, state: State) -> State:
        probs = self._normalize_probs(state["engaged"], state["confused"], state["frustrated"], state["bored"])
        state["engaged"], state["confused"], state["frustrated"], state["bored"] = probs
        return state
    ...
    def _initial_emotions_for_archetype(self, archetype: str) -> Tuple[float, float, float, float]:
        if archetype == "anxious":
            return 0.33 + self.rng.uniform(-0.05, 0.05), 0.26 + self.rng.uniform(-0.04, 0.04), 0.30 + self.rng.uniform(
                -0.04, 0.04
            ), 0.11 + self.rng.uniform(-0.03, 0.03)
        if archetype == "boredom_prone":
            return 0.30 + self.rng.uniform(-0.05, 0.05), 0.14 + self.rng.uniform(-0.03, 0.03), 0.12 + self.rng.uniform(
                -0.03, 0.03
            ), 0.44 + self.rng.uniform(-0.05, 0.05)
        if archetype == "struggling":
            return 0.28 + self.rng.uniform(-0.05, 0.05), 0.34 + self.rng.uniform(-0.05, 0.05), 0.26 + self.rng.uniform(
                -0.04, 0.04
            ), 0.12 + self.rng.uniform(-0.03, 0.03)
        return (
            0.45 + self.rng.uniform(-0.08, 0.08),
            0.20 + self.rng.uniform(-0.05, 0.05),
            0.20 + self.rng.uniform(-0.05, 0.05),
            0.15 + self.rng.uniform(-0.05, 0.05),
        )
    ...
    def _adjust_correct_prob_for_archetype(self, correct_prob: float, action: str, prev: State) -> float:
        if self.archetype == "anxious":
            if action == "question_hard":
                correct_prob -= 0.10
            if action == "question_review":
                correct_prob += 0.06
        elif self.archetype == "boredom_prone":
            if action == "question_easy":
                correct_prob -= 0.06
            if action == "question_medium":
                correct_prob += 0.03
        elif self.archetype == "struggling":
            correct_prob -= 0.08
            if action == "question_review":
                correct_prob += 0.05
            if prev["confused"] > 0.30:
                correct_prob -= 0.05
        return correct_prob
