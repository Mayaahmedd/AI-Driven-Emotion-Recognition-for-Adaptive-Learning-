...
from adaptation.step3.baselines import BaselinePolicy
...
class Evaluator:
    ...
    def evaluate(self, episodes: int = 100) -> EvalStats:
        ...
        for _ in range(episodes):
            ...
            while not done:
                ...
                vec = self.simulator.to_vector(state)
                agent = self.explanation_agent if phase == "explanation" else self.practice_agent
                action = agent.choose_action(vec, explore=False)
    ...
    def compare_policies(self, baselines: List[BaselinePolicy], episodes_per_archetype: int = 80) -> List[ComparisonRow]:
        ...
        all_archetypes = self.simulator.config.eval_archetypes or ARCHETYPES
        for archetype in all_archetypes:
            ...
            rows.append(
                ComparisonRow(
                    policy="dqn",
                    archetype=archetype,
                    avg_reward=rl_stats.avg_reward,
                    avg_correct_rate=rl_stats.avg_correct_rate,
                    avg_engagement=rl_stats.avg_engagement,
                    avg_frustration=rl_stats.avg_frustration,
                    avg_boredom=rl_stats.avg_boredom,
                )
            )
            for baseline in baselines:
                ...
                rows.append(
                    ComparisonRow(
                        ...
                        avg_frustration=stats.avg_frustration,
                        avg_boredom=stats.avg_boredom,
                    )
                )
        return rows
