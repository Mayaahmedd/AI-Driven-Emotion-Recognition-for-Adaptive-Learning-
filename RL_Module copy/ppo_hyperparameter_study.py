"""
PPO hyperparameter optimization study (environment fixed).

Search space is sized to match the completed DQN factorial grid (2304 configs):
  learning_rate x batch_size x n_steps x clip_range x ent_coef x n_epochs

Phases:
  baseline  - reproduce default PPO across 5 seeds
...
Run:
  python -m RL_Module.ppo_hyperparameter_study --phase all
  python -m RL_Module.ppo_hyperparameter_study --phase grid --resume --workers 4
"""
...
OUT_DIR = _HERE / "figures" / "ppo_hyperparameter_study"
OUT_DIR.mkdir(parents=True, exist_ok=True)

TRAIN_TS = cfg.TOTAL_TIMESTEPS
EVAL_EPS = cfg.EVAL_EPISODES
ROBUST_SEEDS = [42, 123, 999, 2024, 777]
SCREEN_SEED = 42

# Analogous dimensions to DQN study (4*4*4*3*4*3 = 2304)
SEARCH_SPACE = {
    "learning_rate": [1e-4, 3e-4, 5e-4, 1e-3],
    "batch_size": [32, 64, 128, 256],
    "n_steps": [512, 1024, 2048, 4096],
    "clip_range": [0.1, 0.2, 0.3],
    "ent_coef": [0.001, 0.01, 0.05, 0.1],
    "n_epochs": [3, 5, 10],
}
...
def run_baseline() -> pd.DataFrame:
    print(f"\n=== Section 1: PPO baseline ({len(ROBUST_SEEDS)} seeds) ===")
    rows = []
    for seed in ROBUST_SEEDS:
        rows.append(train_and_eval(BASELINE_HP, seed, f"baseline_s{seed}"))
...
def _hp_from_row(row: pd.Series) -> Dict[str, Any]:
    float_keys = {"learning_rate", "clip_range", "ent_coef"}
    return {
...
def educational_validity() -> Dict[str, Any]:
    ...
    for key in ("best_reward", "best_mastery", "best_balanced"):
        ...
        if reward_up and not adapt_up and not comfort_down:
...
def final_recommendation() -> Dict[str, Any]:
    ...
    rec["note"] = (
        ...
        "only PPO algorithm hyperparameters were tuned."
    )
...
def build_report() -> None:
    lines = ["# PPO Hyperparameter Optimization Report\n"]
    ...
    (OUT_DIR / "PPO_HYPERPARAMETER_REPORT.md").write_text("".join(lines))
    print(f"Wrote {OUT_DIR / 'PPO_HYPERPARAMETER_REPORT.md'}")


def main() -> None:
    parser = argparse.ArgumentParser(description="PPO hyperparameter study")
    parser.add_argument(
        "--phase",
    ...
    print(f"PPO study | train={TRAIN_TS} eval={EVAL_EPS} | grid={n_grid} configs")

    if args.phase in ("baseline", "all"):
        run_baseline()
    if args.phase in ("grid", "all"):
        run_grid(resume=args.resume, max_configs=args.max_configs, workers=args.workers)
