...
def generate_population(
    ...
    for i in range(n):
        student = SyntheticStudent(
            ...
            frustration_tolerance=float(rng.uniform(0.2, 1.0)),
            boredom_sensitivity=float(rng.uniform(0.2, 1.0)),
            engagement_recovery=float(rng.uniform(0.2, 0.9)),
            confidence=float(rng.uniform(-0.2, 0.3)),
            persistence=float(rng.uniform(0.1, 0.9)),
            student_id=i,
        )
