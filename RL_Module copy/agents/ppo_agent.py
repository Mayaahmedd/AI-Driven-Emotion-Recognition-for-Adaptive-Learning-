...
class PPOAgent(BaseAgent):
    ...
    def train(
        ...
        hp = hyperparams or {}
        default_arch = dict(pi=[256, 256, 256], vf=[256, 256, 256])
        policy_kwargs = dict(net_arch=hp.get("net_arch", default_arch))
        self.model = MaskablePPO(
            "MlpPolicy",
            env,
            learning_rate=hp.get("learning_rate", config.PPO_LEARNING_RATE),
            gamma=hp.get("gamma", config.PPO_GAMMA),
            clip_range=hp.get("clip_range", config.PPO_CLIP_RANGE),
            n_steps=hp.get("n_steps", config.PPO_N_STEPS),
            batch_size=hp.get("batch_size", config.PPO_BATCH_SIZE),
            ent_coef=hp.get("ent_coef", config.PPO_ENT_COEF),
