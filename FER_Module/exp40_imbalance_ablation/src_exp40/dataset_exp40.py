"""
DAiSEE multilabel dataset for exp40 (imbalance ablation).

This is a byte-faithful re-implementation of the exp38 dataset.
The reason it is *not* the per-clip / MTCNN refactor used in exp39 is
methodological: exp40 ablates ONLY the imbalance-correction strategy
(oversampling vs pos_weight vs both). If we changed the augmentation or
spatial-crop policy at the same time, the resulting comparison would
confound two independent design dimensions. By holding the data pipeline
fixed to exp38's pipeline, the only quantity that varies across
variants A / B / C is the imbalance correction.
"""
...
class DAiSEEMultilabelDatasetExp40(Dataset):
